                ...
                for index, link in enumerate(node_link):
                    ...
                    if link is not None:
                        ...
                        if link_destination == s and check_node == d:
                            ...
                            # # print(S[link_destination][link_loc_node][link_loc_qubit])
                            # S[link_destination][link_loc_node][link_loc_qubit] = None
                            execute_list.append([age, [check_node, node, index], [link_destination, link_loc_node, link_loc_qubit]])
                        elif link_destination == d and check_node == s:
                            # print(S[check_node][node][index])
                            # S[check_node][node][index] = None
                            # print(S[link_destination][link_loc_node][link_loc_qubit])
...
def time_evolution_greedy(srs_configurations, circuit_dagtable, gate_list, qubit_loc_subcircuit_dic, subcircuits_allocation,
            ...
            for gate_id in current_gate:
                if gate_id not in remote_operations:
                    ...
                    if schedule == 0:
                        ...
                        path, S = execute_remote_operation_random_path(qubit1_loc, qubit2_loc, S, vadj, max_links_swapped)
                    if path is not None:
                        executed_gate_list.append(gate_id)
                        ecost += len(path) / 2
                        for qubit in qubits:
...
def time_evolution_old(srs_configurations, circuit_dagtable, gate_list, qubit_loc_subcircuit_dic, subcircuits_allocation,
                   remote_operations, schedule):
    """
    :param circuit_dagtable: 量子线路的dagtable, 用来配合get_front_layer获取当前最新需要执行的量子门
    :param gate_list: 量子线路门列表，用于从id到门信息的映射
...
def time_evolution_only_remote_time(srs_configurations, circuit_dagtable, gate_list, qubit_loc_subcircuit_dic, subcircuits_allocation,
                   remote_operations, schedule):
    """
    :param circuit_dagtable: 量子线路的dagtable, 用来配合get_front_layer获取当前最新需要执行的量子门
    :param gate_list: 量子线路门列表，用于从id到门信息的映射
...
def time_evolution_old_only_remote_time(srs_configurations, circuit_dagtable, gate_list, qubit_loc_subcircuit_dic, subcircuits_allocation,
                   remote_operations, schedule):
    """
    :param circuit_dagtable: 量子线路的dagtable, 用来配合get_front_layer获取当前最新需要执行的量子门
    :param gate_list: 量子线路门列表，用于从id到门信息的映射
        ...
        for file in files:
            if file.endswith('.qasm'):
                ...
                for i in range(num_sample):
                    ...
                    ecost, time_step, execution_schedule, discard_entanglement_count = time_evolution(srs_configurations, circuit_dagtable,
