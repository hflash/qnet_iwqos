    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            if file.endswith('.qasm'):
                ...
                if "rca" in qasm_path or "qaoa" in qasm_path or "qft" in qasm_path:
                    continue
                ...
                for i in range(num_sample):
                    path_write = '../exp_data/baseline_0221_unweighted/' + scale + '/' + circuitname + '_shortest_path' + "_trivial_allocation" + "_bandwidth_" + str(qubit_per_channel) + "_without_old" + '.txt'
                    print(path_write)
                    ...
                    timestart = time.time()
                    ecost, time_step, execution_schedule, discard_entanglement_count = dioc.time_evolution(
