...
def batch_circuit_execution(schedule, qswap, cutoff, qubit_per_channel, num_sample, small_device_qubit_number, large_device_qubit_number):
    ...
    path = '../exp_circuit_benchmark/large_scale/qaoa'
    # path = './'
    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            if file.endswith('.qasm'):
                ...
                if '100' not in qasm_path:
                    continue
                for i in range(num_sample):
                    # todo: change the write file path
                    path_write = '../exp_data/sens_cutoff_qswap/' + scale + '/' + circuitname + '_cutoff_' + str(cutoff) +'_qswap_'+ str(qswap)+'_shortest_path_' + "hungarian_allocation" + "_bandwidth_" + str(qubit_per_channel) + '_old' + '.txt'
                    print(path_write)
        ...
        for cutoff in cutoffs:
            for qswap in q_swaps:
                for qubit_per_channel in qubit_per_channels:
                    batch_circuit_execution(schedule, qswap, cutoff, qubit_per_channel, N_samples, small_device_qubit_number, large_device_qubit_number)
