    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            if file.endswith('.qasm'):
                ...
                if 'rca_100' in qasm_path or 'rca_200' in qasm_path:
                    continue
                for i in range(num_sample):
                    # todo: change the write file path
                    path_write = '../exp_data/baseline_0221_unweighted_only_remote/' + scale + '/' + circuitname + '_shortest_path_' + "trivial_allocation" + "_bandwidth_" + str(qubit_per_channel) + '_old_' + '.txt'
                    print(path_write)
